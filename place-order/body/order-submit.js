// =======================================================================
//  order-submit.js  –  Submit order to middleware, handle UX + session
//  Requires order-submit.css for spinner / disabled button styling.
// =======================================================================
...
function setSessionDataWithExpiry(k,v,min){
  sessionStorage.setItem(k,JSON.stringify({value:v,expiry:Date.now()+min*60000}));
}
function getSessionDataWithExpiry(k){
  const itm=JSON.parse(sessionStorage.getItem(k)||"null");
  if(!itm||Date.now()>itm.expiry){sessionStorage.removeItem(k);return null;}
  return itm.value;
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  function validateForm(fd){
    ...
    Object.entries(req).forEach(([n,msg])=>{
  ...
  orderBtn.addEventListener('click',async(evt)=>{
    ...
    const shippingFee=document.querySelector('#shippingfee')?.value.trim()||"";
    const jsonData={
