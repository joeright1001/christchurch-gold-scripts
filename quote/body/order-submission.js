...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  const orderBtn=document.querySelector('#get-quote-submit');
  ...
  orderBtn.addEventListener('click',async(evt)=>{
    ...
    let shippingFee = "0";
    if (document.querySelector('#shippingfee')?.checked) {
      shippingFee = document.querySelector('#shippingfee-amount')?.textContent.trim() || "25";
    }
    const jsonData={
      /* customer */
    ...
    try{
      ...
      if(out.token&&out.trade_order){
        ...
        window.location.href = `/complete-quote?trade_order=${out.trade_order}`;
      }else{
